...
7. 排序 & 搜索
    ...
    + 希尔排序
      + 第一次打破了排序算法时间复杂度 O(N²) 的限制。
      + 希尔排序的思想：先初始化一个间隔，对数组中对应间隔的数进行两两排序；缩小间隔数，继续以相同规则进行排序；直到最后一次间隔为 1 再进行一次排序后的数组就是有序数组了。
      + 间隔选择：初始间距为 N / 2，N 为数组的长度；Hibbard 增量序列，增量算法为 x^k - 1，这种增量最坏复杂度为 O(N^3/2)，猜想的平均复杂度为 O(N^5/4)；Sedgewick 增量序列，{1, 5, 19, 41, 109,...}，其中梅香为 94^i - 9 * 2^i 或 4^i - 32^i + 1，最坏的复杂度为 O(N^4/3)，平均复杂度为 O(N^7/6)，但未被证明。
      + 希尔排序的效率：希尔排序的效率跟增量有关系，它的效率证明非常困难，但是经过统计，希尔排序使用原始增量最坏的情况下时间复杂度为 O(N²)，通常情况下都要好于 O(N²)。
    + 快速排序
      + 快速排序的思想：快速排序的主要思想是分而治之，从所有的数字中任意选出一个数字，然后将大于该数字的数字放到该数字的右侧，小于该数字的数字放到该数字的左侧，这样一次下来这个数字的位置就是最终位置；然后依次递归处理左右两侧的其他数字，则最终得到有序数列。
      + 快速排序的枢纽：快速排序中一个很重要的步骤就是选取枢纽（pivot，也称为主元）；一种方案是直接选择第一个元素作为枢纽，但是这种情况下效率不高；另一种方案是使用随机数，但是随机函数本身就是一个壕性能的操作；另外一种比较优秀的解决方案是取头、中、尾的中位数。
      + 快速排序最坏情况下就是枢纽选在数组的边界上，那么此时的效率等于冒泡排序，但是因为采取中位值作为枢纽所以不可能遇到这种情况。快速排序的平均效率是 O(N * logN)。
